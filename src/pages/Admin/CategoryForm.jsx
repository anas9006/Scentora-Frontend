...
const CategoryForm = ({ isOpen, onClose, onSuccess, category }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await categoryAPI.updateCategory(category._id, formDataToSend)
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-3 md:p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <motion.div
        ...
        className="surface-panel w-full max-w-md rounded-lg md:rounded-2xl p-4 md:p-8 my-4"
      >
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <h2 className="text-xl md:text-2xl font-bold gold-text">{isEditing ? 'Edit Category' : 'Add Category'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
          <div>
            <label className="block text-xs md:text-sm text-gray-400 mb-2">Category Name</label>
            <input
              type="text"
              required
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="w-full bg-white/5 border border-secondary/20 rounded-lg px-3 md:px-4 py-2 md:py-3 text-sm focus:outline-none focus:border-secondary transition"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-white/5 border border-secondary/20 rounded-lg px-3 md:px-4 py-2 md:py-3 text-sm focus:outline-none focus:border-secondary transition resize-none"
          ...
          <div>
            <label className="block text-xs md:text-sm text-gray-400 mb-2">Category Image (Optional)</label>
            <div
              className={`relative border-2 border-dashed rounded-lg md:rounded-xl p-4 md:p-6 text-center transition-colors ${
                dragActive
              ...
              <div className="flex flex-col items-center gap-2 md:gap-3">
                ...
                <div>
                  <p className="text-xs md:text-sm font-medium text-white">
                    {dragActive ? 'Drop image here' : `${isEditing ? 'Replace' : 'Drag & drop'} category image`}
                  </p>
            ...
            <div className="flex items-center gap-2 text-xs md:text-sm text-gray-400 mt-2">
              <FiImage />
              <span>{selectedImage ? '1 new image selected' : imagePreview ? 'Current image selected' : 'No image selected'}</span>
            </div>
          </div>
          ...
          <div className="flex gap-2 md:gap-4 pt-2 md:pt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 md:px-6 py-2 md:py-3 border border-secondary/20 rounded-lg font-semibold text-sm md:text-base hover:bg-white/5 transition"
            >
              Cancel
            </button>
            <button
            ...
            >
              {loading ? (
                ...
                <>
                  <FiCheck size={16} /> {isEditing ? 'Update' : 'Create'}
                </>
              )}
            </button>
          </div>
